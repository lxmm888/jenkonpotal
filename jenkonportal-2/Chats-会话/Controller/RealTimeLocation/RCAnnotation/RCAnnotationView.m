...
- (id)initWithAnnotation:(id<MKAnnotation>)annotation {
  ...
  if (self) {
    ...
    [_imageView
        sd_setImageWithURL:[NSURL URLWithString:[self.imageUrl WFYURLString] ]
          placeholderImage:[AppSession imageNamed:@"default_portrait_msg"
      ...
      if (![[change objectForKey:@"new"] isKindOfClass:[NSNull class]]) {
        [_imageView
            sd_setImageWithURL:[NSURL
                                   URLWithString:[[change objectForKey:@"new"] WFYURLString]]
              placeholderImage:[AppSession imageNamed:@"default_portrait_msg"
                                               ofBundle:@"RongCloud.bundle"]];
      }
    });
