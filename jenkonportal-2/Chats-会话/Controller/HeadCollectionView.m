...
- (instancetype)initWithFrame:(CGRect)frame
  ...
  if (self) {
    ...
    [self setBackgroundColor:[UIColor colorWithRed:0.5
                                             green:0.5
                                              blue:0.5
                                             alpha:0.5]];
      ...
      [[RCIM sharedRCIM]
          ...
          getUserInfoWithUserId:userId
                     completion:^(RCUserInfo *userInfo) {
                       ...
                       if (show) {
                         [self showUserChangeInfo:
                                   [NSString stringWithFormat:@"%@加入...",
                                                              userInfo.name]];
                       } else {
                         self.tipLabel.text = [NSString
                             stringWithFormat:@"%lu人在共享位置",
                                              (unsigned long)
...
- (void)addHeadViewUser:(RCUserInfo *)user {
  {
    ...
    [userHead
        sd_setImageWithURL:[NSURL URLWithString:[user.portraitUri WFYURLString]]
          placeholderImage:[RCKitUtility imageNamed:@"default_portrait_msg"
