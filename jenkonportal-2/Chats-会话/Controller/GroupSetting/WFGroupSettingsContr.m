...
static NSString *CellIdentifier = @"WFYBaseSettingTableViewCell";

@interface WFGroupSettingsContr ()
//开始会话
...
@implementation WFGroupSettingsContr
{
    UITableView *_tbGroup;
...
- (void)setGroup:(WFGroupInfo *)Group
{
    _Group = Group;
    if (_Group) {
        groupId =[NSString stringWithFormat:@"%@",_Group.groupId];
        if (creatorId == nil) {
...
- (void)createUI{
    _tbGroup = [[UITableView alloc]init];
    [self.view addSubview:_tbGroup];
}
 ...
 */
- (void)bindHeaderView{
    CGRect tempRect = CGRectMake(0, 0, SCREEN_WIDTH, headerView.collectionViewLayout.collectionViewContentSize.height);
    ...
    //注册cell
    [headerView registerClass:[WFGroupSettingsContrHeaderItem class]
   forCellWithReuseIdentifier:@"WFGroupSettingsContrHeaderItem"];
    [headerView registerClass:[WFGroupSettingsContrHeaderItem class]
   forCellWithReuseIdentifier:@"WFGroupSettingsContrHeaderItemForSigns"];
}
...
- (void)bindUIValue{    
    ...
    _tbGroup.userInteractionEnabled = YES;
    _tbGroup.delegate= self;
    _tbGroup.dataSource= self;
    _tbGroup.tableFooterView = [[UIView alloc]init];
    _tbGroup.backgroundColor = HEXCOLOR(0xf0f0f6);
    _tbGroup.separatorColor = HEXCOLOR(0xdfdfdf);
    _tbGroup.separatorStyle = UITableViewCellSeparatorStyleNone;
...
- (void)layoutUI{
    ...
    CGFloat groupW = SCREEN_WIDTH, groupH = SCREEN_HEIGHT-groupTopMargin, groupX = 0, groupY = 0;
    _tbGroup.frame = CGRectMake(groupX, groupY, groupW, groupH);
}

//bindUIValue2在viewWillAppear的时候调用
- (void)bindUIValue2{
...
- (void)startLoad{
    ...
    NSMutableArray *groupMemberList = [[WFDataBaseManager shareInstance]getGroupMember:groupId];
    ...
    //发送网络请求，根据groupId获取群组成员信息
    [[WFHttpTool shareInstance]
     getGroupMembersWithGroupId:groupId Block:^(NSMutableArray *result) {
         if ([result count]>0) {
             dispatch_async(dispatch_get_main_queue(), ^{
                 weakSelf.title = [NSString stringWithFormat:@"群组信息(%lu)",(unsigned long)result.count];
                 WFYGroupSettingsTableViewCell *cell=
                 (WFYGroupSettingsTableViewCell *)
                 [_tbGroup viewWithTag:WFYGroupSettingsTableViewCellGroupNameTag];
             ...
             collectionViewResource = [tempArray mutableCopy];
             [self setHeaderView];
             [[WFDataBaseManager shareInstance]
              insertGroupMemberToDB:result groupId:groupId complete:^(BOOL results) {
                  
              }];
             for (WFYUserInfo *user in result) {
                 [[RCIM sharedRCIM] refreshUserInfoCache:user withUserId:user.userId];
             }
         }
     }];
    /******************添加footerview*******************/
    [self bindTableFooterView];
}

- (void) bindTableFooterView{
...
- (void)setHeaderView
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        _tbGroup.tableHeaderView = [[UIView alloc] initWithFrame:frame];
        _tbGroup.tableHeaderView.backgroundColor = [UIColor whiteColor];
        [_tbGroup.tableHeaderView addSubview:headerView];
        ...
        [_tbGroup.tableHeaderView addSubview:separatorLine];
    });
}
...
//                            }];
    dispatch_async(dispatch_get_main_queue(), ^{
        WFYBaseSettingTableViewCell *cell =
        (WFYBaseSettingTableViewCell *)[_tbGroup viewWithTag:WFYGroupSettingsTableViewCellGroupPortraitTag];
        [cell.rightImageView sd_setImageWithURL:[NSURL URLWithString:[self.Group.portraitUri WFYURLString]]];
        //    cell.PortraitImg.image = image;
    });
}
...
clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (actionSheet.tag == 100) {
        if (buttonIndex == 0) {
            NSIndexPath *indexPath = [_tbGroup indexPathForSelectedRow];
            WFYBaseSettingTableViewCell *cell = [_tbGroup cellForRowAtIndexPath:indexPath];
    ...
    if (actionSheet.tag == 101) {
        if (buttonIndex == 0) {
            ...
            [WFHTTPTOOL quitGroupWithUserId:kUserId GroupId:groupId complete:^(BOOL isOk) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (isOk) {
                        [[RCIMClient sharedRCIMClient]
                         clearMessages:ConversationType_GROUP
    ...
    if (actionSheet.tag == 102) {
        if (buttonIndex == 0) {
            [WFHTTPTOOL
             dismissGroupWithUserId:kUserId
             GroupId:groupId
             complete:^(BOOL isOk) {
                 ...
                 dispatch_async(dispatch_get_main_queue(), ^{
                     if (isOk) {
                         ...
                         [[RCIMClient sharedRCIMClient]
                          removeConversation:ConversationType_GROUP
                          targetId:groupId];
                         [[WFDataBaseManager shareInstance] deleteGroupToDB:groupId];
                         [self.navigationController
                          popToRootViewControllerAnimated:YES];
                     } else {
                         UIAlertView *alertView = [[UIAlertView alloc]
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    WFYGroupSettingsTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    if(!cell){
        cell = [[WFYGroupSettingsTableViewCell alloc]initWithIndexPath:indexPath andGroupInfo:_Group];
    }
    //设置switchbutton的开关状态
...
didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    switch (indexPath.section) {
        case 0:{
            //进入群组成员信息展示页面
            WFYGroupMembersContr *GroupMembersVC =
            [[WFYGroupMembersContr alloc] init];
            NSMutableArray *groupMemberList = [[WFDataBaseManager shareInstance] getGroupMember:_Group.groupId];
            groupMemberList = [self moveCreator:groupMemberList];
        ...
        case 1:{
            switch (indexPath.row) {
                ...
                case 1:
                {
                    if (isCreator == YES) {
                        //如果是创建者，进入修改群名称页面
                        WFYEditGroupNameContr *editGroupNameVC = [WFYEditGroupNameContr editGroupNameContr];
                ...
                case 2:
                {
                    WFYGroupAnnouncementContr *vc = [[WFYGroupAnnouncementContr alloc] init];
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
    ...
    WFGroupSettingsContrHeaderItem *cell =
    [collectionView dequeueReusableCellWithReuseIdentifier:@"WFGroupSettingsContrHeaderItem"
    ...
    if (collectionViewResource.count > 0) {
        ...
        if (![collectionViewResource[indexPath.row]
            ...
            WFGroupSettingsContrHeaderItem *cellForSigns = [collectionView dequeueReusableCellWithReuseIdentifier:
                                                                       @"WFGroupSettingsContrHeaderItemForSigns"
                                                                                                                forIndexPath:indexPath];
...
didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    contactSelectedVC.groupId = [NSString stringWithFormat:@"%@",_Group.groupId];
    ...
    if (isCreator == YES) {
        ...
        if (indexPath.row == collectionViewResource.count - 2) {
            ...
            contactSelectedVC.delGroupMembers = nil;
        ...
        if (indexPath.row == collectionViewResource.count - 1) {
            ...
            contactSelectedVC.addGroupMembers = nil;
            [self.navigationController pushViewController:contactSelectedVC
                                                 animated:YES];
            return;
        }
    ...
    staff.usrname=selectedUser.userId;
    staff.nickname = selectedUser.name;
    staff.avatarUri = selectedUser.portraitUri;
    c.staff = staff;
...
- (void)didReceiveMessageNotification:(NSNotification *)notification{
    ...
    if ([message.content isMemberOfClass:[RCGroupNotificationMessage class]]) {
        ...
        if ([groupNotification.operation isEqualToString:@"Dismiss"]) {
            ...
            //发送网络请求，根据groupId获取群组成员信息，更新本地数据库群组成员信息
            [WFHTTPTOOL getGroupMembersWithGroupId:message.targetId
...
- (void)refreshTabelViewInfo {
    //  发送网络请求，根据id获取单个群组信息，然后刷新群组信息
    [WFHTTPTOOL getGroupByID:groupId
...
#pragma mark - WFYBaseSettingTableViewCellDelegate
